...
def get_email_body(msg):
    ...
    if msg.is_multipart():
        ...
        try:
            ...
            # Ignore decoding errors for non-text payloads
            pass
    return body
...
async def delete_alert(alert_id: int, current_user: dict = Depends(get_current_user)):
    ...
    async with db_pool.acquire() as conn:
        await conn.execute(
            "DELETE FROM tracker_alerts WHERE id = $1",
...
async def toggle_favorite(alert_id: int, current_user: dict = Depends(get_current_user)):
    ...
    async with db_pool.acquire() as conn:
        current = await conn.fetchrow(
            "SELECT favorite FROM tracker_alerts WHERE id = $1",
            alert_id, current_user['id']
        )
...
async def add_bike_note(
    ...
    async with db_pool.acquire() as conn:
        ...
        note = await conn.fetchrow(
            ...
            RETURNING *
            """,
            bike_id, request.note, current_user.get('username', 'User')
        )
